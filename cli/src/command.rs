...
pub use digital_circulation_performance_test::PerfCheckError;
...
impl SubstrateCli for Cli {
	fn impl_name() -> String {
		"Parity DigitalCirculation".into()
	}
	...
	fn executable_name() -> String {
		"digital_circulation".into()
	}

	fn load_spec(&self, id: &str) -> std::result::Result<Box<dyn sc_service::ChainSpec>, String> {
		let id = if id == "" {
			let n = get_exec_name().unwrap_or_default();
			["digital_circulation", "kusama", "westend", "rococo", "versi"]
				.iter()
				.cloned()
				.find(|&chain| n.starts_with(chain))
				.unwrap_or("digital_circulation")
		} else {
			id
		};
		Ok(match id {
			...
			"digital_circulation" => Box::new(service::chain_spec::digital_circulation_config()?),
			#[cfg(feature = "digital_circulation-native")]
			"digital_circulation-dev" | "dev" => Box::new(service::chain_spec::digital_circulation_development_config()?),
			#[cfg(feature = "digital_circulation-native")]
			"digital_circulation-local" => Box::new(service::chain_spec::digital_circulation_local_testnet_config()?),
			#[cfg(feature = "digital_circulation-native")]
			"digital_circulation-staging" => Box::new(service::chain_spec::digital_circulation_staging_testnet_config()?),
			...
			path => {
				...
				let chain_spec = Box::new(service::DigitalCirculationChainSpec::from_json_file(path.clone())?)
	...
	fn native_runtime_version(spec: &Box<dyn service::ChainSpec>) -> &'static RuntimeVersion {
		...
		#[cfg(feature = "digital_circulation-native")]
		{
			return &service::digital_circulation_runtime::VERSION
		}

		#[cfg(not(feature = "digital_circulation-native"))]
		panic!("No runtime feature (digital_circulation, kusama, westend, rococo) is enabled")
	}
}
...
fn set_default_ss58_version(spec: &Box<dyn service::ChainSpec>) {
	let ss58_version = if spec.is_kusama() {
		...
		Ss58AddressFormatRegistry::DigitalCirculationAccount
	}
	.into();
...
const DEV_ONLY_ERROR_PATTERN: &'static str =
	"can only use subcommand with --chain [digital_circulation-dev, kusama-dev, westend-dev, rococo-dev, wococo-dev], got ";
...
/// Unwraps a [`digital_circulation_client::Client`] into the concrete runtime client.
macro_rules! unwrap_client {
	(
		...
		match $client.as_ref() {
			#[cfg(feature = "digital_circulation-native")]
			digital_circulation_client::Client::DigitalCirculation($client) => $code,
			#[cfg(feature = "westend-native")]
			digital_circulation_client::Client::Westend($client) => $code,
			#[cfg(feature = "kusama-native")]
			digital_circulation_client::Client::Kusama($client) => $code,
			#[cfg(feature = "rococo-native")]
			digital_circulation_client::Client::Rococo($client) => $code,
...
/// Parses digital_circulation specific CLI arguments and run the service.
pub fn run() -> Result<()> {
	...
	match &cli.subcommand {
		None => run_node_inner(cli, service::RealOverseerGen, digital_circulation_node_metrics::logger_hook()),
		...
		Some(Subcommand::ExportBlocks(cmd)) => {
			...
			Ok(runner.async_run(|mut config| {
				let (client, _, _, task_manager) =
					service::new_chain_ops(&mut config, None).map_err(Error::DigitalCirculationService)?;
		...
		Some(Subcommand::PvfPrepareWorker(cmd)) => {
			...
			{
				digital_circulation_node_core_pvf::prepare_worker_entrypoint(&cmd.socket_path);
				Ok(())
			}
		},
		Some(Subcommand::PvfExecuteWorker(cmd)) => {
			...
			#[cfg(not(target_os = "android"))]
			{
				digital_circulation_node_core_pvf::execute_worker_entrypoint(&cmd.socket_path);
				Ok(())
			}
		},
			...
			match cmd {
				...
				BenchmarkCmd::Overhead(cmd) => {
					...
					runner.sync_run(|mut config| {
						use digital_circulation_client::benchmark_inherent_data;
						let (client, _, _, _) = service::new_chain_ops(&mut config, None)?;
				...
				BenchmarkCmd::Pallet(cmd) => {
					...
					// else we assume it is digital_circulation.
					#[cfg(feature = "digital_circulation-native")]
					{
						return Ok(runner.sync_run(|config| {
							cmd.run::<service::digital_circulation_runtime::Block, service::DigitalCirculationExecutorDispatch>(config)
								.map_err(|e| Error::SubstrateCli(e))
						})?)
					}
				...
				// NOTE: this allows the Digital Circulation client to leniently implement
		...
		Some(Subcommand::TryRuntime(cmd)) => {
			...
			#[cfg(feature = "digital_circulation-native")]
			{
				return runner.async_run(|config| {
					Ok((
						cmd.run::<service::digital_circulation_runtime::Block, service::DigitalCirculationExecutorDispatch>(
							config,
						)
						.map_err(Error::SubstrateCli),
						task_manager,

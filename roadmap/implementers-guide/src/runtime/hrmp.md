...
Those entry-points are meant to be called from a dcchain. `origin` is defined as the `ParaId` of
the dcchain executed the message.
...
* `hrmp_init_open_channel(recipient, proposed_max_capacity, proposed_max_message_size)`:
    ...
    channels (`config.hrmp_max_dcchain_outbound_channels` or `config.hrmp_max_parathread_outbound_channels`) minus 1.
...
* `hrmp_accept_open_channel(sender)`:
    ...
    (`config.hrmp_max_dcchain_inbound_channels` or `config.hrmp_max_parathread_inbound_channels`)
    minus 1.

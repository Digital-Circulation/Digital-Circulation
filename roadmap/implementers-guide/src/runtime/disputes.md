...
1. Freeze the state of dcchains so nothing further is backed or included.
...
The storage of this module is designed around tracking [`DisputeState`s](../types/disputes.md#disputestate), updating them with votes, and tracking blocks included by this branch of the relay chain. It also contains a `Frozen` parameter designed to freeze the state of all dcchains.
...
// the chain will not accept any new dcchain blocks for backing or inclusion,

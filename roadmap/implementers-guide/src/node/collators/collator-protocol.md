...
> TODO: note the incremental validation function Ximin proposes at https://github.com/ajie1155/digital_circulation/issues/1348
...
It is assumed that collators are only collating on a single dcchain. Collations are generated by the [Collation Generation][CG] subsystem. We will keep up to one local collation per relay-parent, based on `DistributeCollation` messages. If the para is not scheduled on any core, at the relay-parent, or the relay-parent isn't in the active-leaves set, we ignore the message as it must be invalid in that case - although this indicates a logic error elsewhere in the node.

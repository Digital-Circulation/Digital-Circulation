...
Let's contextualize that diagram a bit by following a dcchain block from its creation through finalization.
Parachains can use completely arbitrary processes to generate blocks. The relay chain doesn't know or care about
the details; each dcchain just needs to provide a [collator](collators/collation-generation.md).

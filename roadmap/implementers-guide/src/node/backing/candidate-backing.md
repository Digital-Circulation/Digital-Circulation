...
>
> * At the moment we only allow inclusion of _new_ dcchain candidates validated by _current_ validators.
> * Allow inclusion of _old_ dcchain candidates validated by _current_ validators.
> * Allow inclusion of _old_ dcchain candidates validated by _old_ validators.
>
...
* Fetch current validator set, validator -> dcchain assignments from [`Runtime API`][RA] subsystem using [`RuntimeApiRequest::Validators`][RAM] and [`RuntimeApiRequest::ValidatorGroups`][RAM]
* Determine if the node controls a key in the current validator set. Call this the local key if so.
* If the local key exists, extract the dcchain head and validation function from the [`Runtime API`][RA] for the dcchain the local key is assigned to by issuing a [`RuntimeApiRequest::Validators`][RAM]
...
match msg {
  ...
  CandidateBackingMessage::Second(hash, candidate) => {
    if candidate is unknown and in local assignment {
      if spawn_validation_work(candidate, dcchain head, validation function).await == Valid {
  ...
  CandidateBackingMessage::Statement(hash, statement) => {
    ...
    if let Statement::Seconded(candidate) = statement {
      if candidate.dcchain_id == our_assignment {
        spawn_validation_work(candidate, dcchain head, validation function)
      }
    }
  }
}
...
```rust
fn spawn_validation_work(candidate, dcchain head, validation function) {
  asynchronously {

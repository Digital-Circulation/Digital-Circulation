...
pub use digital_circulation_core_primitives::v2::{
...
// Export some digital_circulation-dcchain primitives
...
/// The key type ID for a dcchain validator key.
...
/// Identity that dcchain validators use when signing validation messages.
///
/// For now we assert that dcchain validator set is exactly equivalent to the authority set, and
...
// We should really get https://github.com/ajie1155/digital_circulation/issues/2403 going ..
...
/// Signature with which dcchain validators sign blocks.
///
/// For now we assert that dcchain validator set is exactly equivalent to the authority set, and
/// so we define it to be the same type as `SessionKey`. In the future it may have different crypto.
...
pub mod well_known_keys {
	...
	///
	/// The storage entry stores a `Hash`. This is digital_circulation hash which is at the moment
	/// `blake2b-256`.
	...
	/// The signal that indicates whether the dcchain should go-ahead with the proposed validation
	/// code upgrade.
	///
	...
	/// The signal that indicates whether the dcchain is disallowed to signal an upgrade at this
	/// relay-parent.
	///
...
/// The key type ID for dcchain assignment key.
...
/// to approve included dcchain candidates.
mod assignment_app {
	use application_crypto::{app_crypto, sr25519};
...
pub struct CandidateDescriptor<H = Hash> {
	...
	/// The dcchain index, the relay parent, the validation data hash, and the `pov_hash`.
...
impl Ord for CommittedCandidateReceipt {
	fn cmp(&self, other: &Self) -> sp_std::cmp::Ordering {
		...
		// https://github.com/ajie1155/digital_circulation/issues/222
		self.descriptor()
			.para_id
...
pub struct CandidateCommitments<N = BlockNumber> {
	...
	/// Horizontal messages sent by the dcchain.
...
/// Custom validity errors used in Digital Circulation while validating transactions.
#[repr(u8)]
...
/// Abridged version of `HostConfiguration` (from the `Configuration` dcchains host runtime module)
/// meant to be used by a dcchain or PDK such as cumulus.
#[derive(Clone, Encode, Decode, RuntimeDebug, TypeInfo)]
#[cfg_attr(feature = "std", derive(PartialEq))]
pub struct AbridgedHostConfiguration {
	...
	/// Total number of individual messages allowed in the dcchain -> relay-chain message queue.
	pub max_upward_queue_count: u32,
	/// Total size of messages allowed in the dcchain -> relay-chain message queue before which
	...
	/// The minimum period, in blocks, between which dcchains can update their validation code.
	pub validation_upgrade_cooldown: BlockNumber,
	/// The delay, in blocks, before a validation upgrade is applied.
	pub validation_upgrade_delay: BlockNumber,
}

/// Abridged version of `HrmpChannel` (from the `Hrmp` dcchains host runtime module) meant to be
/// used by a dcchain or PDK such as cumulus.
#[derive(Clone, Encode, Decode, RuntimeDebug, TypeInfo)]
#[cfg_attr(feature = "std", derive(PartialEq))]
...
/// A possible upgrade restriction that prevents a dcchain from performing an upgrade.
...
/// A struct that the relay-chain communicates to a dcchain indicating what course of action the
/// dcchain should take in the coordinated dcchain validation code upgrade process.
///
/// This data type appears in the last step of the upgrade process. After the dcchain observes it
...
pub enum UpgradeGoAhead {
	...
	/// submitted by the dcchain. This variant can also be used to prevent upgrades by the governance
	/// should an emergency emerge.
	///
	/// The expected reaction on this variant is that the dcchain will admit this message and
	...
	/// of the relay-parent where the dcchain observed this signal must use the upgraded validation
	/// code.
	#[codec(index = 1)]
	GoAhead,
}

/// Consensus engine id for digital_circulation v1 consensus engine.
pub const POLKADOT_ENGINE_ID: runtime_primitives::ConsensusEngineId = *b"POL1";

/// A consensus log item for digital_circulation validation. To be used with [`POLKADOT_ENGINE_ID`].
#[derive(Decode, Encode, Clone, PartialEq, Eq)]
pub enum ConsensusLog {
	/// A dcchain or parathread upgraded its code.
	#[codec(index = 1)]
	ParaUpgradeCode(Id, ValidationCodeHash),
	/// A dcchain or parathread scheduled a code upgrade.
	#[codec(index = 2)]
	...
	/// invalid dcchain block within its own chain, due to a dispute.
	#[codec(index = 4)]
	Revert(BlockNumber),
}
...
/// An either implicit or explicit attestation to the validity of a dcchain
/// candidate.
...
/// Statements that can be made about dcchain candidates. These are the
/// actual values that are signed.
#[derive(Clone, PartialEq, Eq, RuntimeDebug)]
#[cfg_attr(feature = "std", derive(Hash))]
pub enum CompactStatement {
	/// Proposal of a dcchain candidate.
	Seconded(CandidateHash),
	/// State that a dcchain candidate is valid.
	Valid(CandidateHash),
}

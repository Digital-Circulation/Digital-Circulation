...
pub mod metric_definitions {
	...
	/// Counts dcchain inherent data weights. Use `before` and `after` labels to differentiate
	...
	pub const PARACHAIN_INHERENT_DATA_WEIGHT: CounterVecDefinition = CounterVecDefinition {
		name: "digital_circulation_dcchain_inherent_data_weight",
	...
	pub const PARACHAIN_INHERENT_DATA_BITFIELDS_PROCESSED: CounterDefinition = CounterDefinition {
		name: "digital_circulation_dcchain_inherent_data_bitfields_processed",
		description: "Counts the number of bitfields processed in `enter_inner`.",
	};

	/// Counts the `total`, `sanitized` and `included` number of dcchain block candidates
	/// in `enter_inner`.
	pub const PARACHAIN_INHERENT_DATA_CANDIDATES_PROCESSED: CounterVecDefinition =
		CounterVecDefinition {
			name: "digital_circulation_dcchain_inherent_data_candidates_processed",
			description:
				"Counts the number of dcchain block candidates processed in `enter_inner`.",
			labels: &["category"],
		};
	...
	pub const PARACHAIN_INHERENT_DATA_DISPUTE_SETS_PROCESSED: CounterVecDefinition =
		CounterVecDefinition {
			name: "digital_circulation_dcchain_inherent_data_dispute_sets_processed",
	...
	pub const PARACHAIN_INHERENT_DATA_DISPUTE_SETS_INCLUDED: CounterDefinition =
		CounterDefinition {
			name: "digital_circulation_dcchain_inherent_data_dispute_sets_included",
			description:
				"Counts the number of dispute statements sets included in a block in `enter_inner`.",
		};
	...
	pub const PARACHAIN_CREATE_INHERENT_BITFIELDS_SIGNATURE_CHECKS: CounterVecDefinition =
		CounterVecDefinition {
			name: "digital_circulation_dcchain_create_inherent_bitfields_signature_checks",

...
use digital_circulation_cli::{Error, Result};
use digital_circulation_node_primitives::CollationGenerationConfig;
use digital_circulation_node_subsystem::messages::{CollationGenerationMessage, CollatorProtocolMessage};
use digital_circulation_primitives::v2::Id as ParaId;
use sc_cli::{Error as SubstrateCliError, Role, SubstrateCli};
use sp_core::hexdisplay::HexDisplay;
use test_dcchain_adder_collator::Collator;

/// The dcchain ID to collate for in case it wasn't set explicitly through CLI.
			...
			runner.run_node_until_exit(|config| async move {
				...
				match role {
					...
					_ => {
						...
						let full_node = digital_circulation_service::build_full(
							config,
							digital_circulation_service::IsCollator::Yes(collator.collator_key()),
							None,
							true,
							None,
							None,
							false,
							digital_circulation_service::RealOverseerGen,
						)
						...
						let para_id =
							cli.run.dcchain_id.map(ParaId::from).unwrap_or(DEFAULT_PARA_ID);

						log::info!("Running adder collator for dcchain id: {}", para_id);

...
//! Collator for the adder test dcchain.

use futures::channel::oneshot;
use futures_timer::Delay;
use parity_scale_codec::{Decode, Encode};
use digital_circulation_node_primitives::{
	Collation, CollationResult, CollationSecondedSignal, CollatorFn, MaybeCompressedPoV, PoV,
	Statement,
};
use digital_circulation_primitives::v2::{CollatorId, CollatorPair};
...
use test_dcchain_adder::{execute, hash_state, BlockData, HeadData};
...
/// The state of the adder dcchain.
struct State {
...
/// The collator of the adder dcchain.
pub struct Collator {
...
impl Collator {
	...
	/// Get the SCALE encoded genesis head of the adder dcchain.
	...
	/// Get the validation code of the adder dcchain.
	pub fn validation_code(&self) -> &[u8] {
		test_dcchain_adder::wasm_binary_unwrap()
	}
	...
	///
	/// This collation function can be plugged into the overseer to generate collations for the adder dcchain.
	pub fn create_collation_function(
		...
		Box::new(move |relay_parent, validation_data| {
			...
			let compressed_pov = digital_circulation_node_primitives::maybe_compress_pov(pov);
	...
	/// Wait until `seconded` collations of this collator are seconded by a dcchain validator.
	///
...
mod tests {
	...
	use digital_circulation_dcchain::primitives::{ValidationParams, ValidationResult};
	use digital_circulation_primitives::v2::PersistedValidationData;
	...
	fn validate_collation(collator: &Collator, parent_head: HeadData, collation: Collation) {
		use digital_circulation_node_core_pvf::testing::validate_candidate;

...
//! Collator for the `Undying` test dcchain.
...
use digital_circulation_primitives::v2::{CollatorId, CollatorPair, Hash};
...
use test_dcchain_undying::{execute, hash_state, BlockData, GraveyardState, HeadData};
...
/// The state of the undying dcchain.
struct State {
...
/// The collator of the undying dcchain.
pub struct Collator {
...
impl Collator {
	...
	/// Get the SCALE encoded genesis head of the dcchain.
	...
	/// Get the validation code of the undying dcchain.
	pub fn validation_code(&self) -> &[u8] {
		test_dcchain_undying::wasm_binary_unwrap()
	}
	...
	///
	/// This collation function can be plugged into the overseer to generate collations for the undying dcchain.
...
mod tests {
	...
	use digital_circulation_primitives::v2::{Hash, PersistedValidationData};
